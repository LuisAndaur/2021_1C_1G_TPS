...
int ll_remove(LinkedList* this,int index)
{
    ...
    if (this!=NULL && index>-1 && index<ll_len(this))
    {
        if (index>0)
        {
            pActual = getNode(this,index);
            auxNode = getNode(this,index-1);
            if (auxNode!=NULL && pActual!=NULL){
                auxNode->pNextNode = pActual->pNextNode;
            }
        }
        else
        {
            pActual = getNode(this,index);
            if (auxNode!=NULL && pActual!=NULL){
                this->pFirstNode = pActual->pNextNode;
            }
        }
        this->size--;
        free(pActual);
    ...
    if (this!=NULL && pFunc!=NULL && order>=0 && order<2)
    {
        ...
        for (int i=0;i<len-1;i++)
        {
            for (int j=i+1;j<len;j++)
            {
                ...
                if((pFunc(elemento1,elemento2) < 0 && !order) || (pFunc(elemento1,elemento2) > 0 && order))
                {
 ...
 * \return LinkedList* Una lista con los elementos que pasaron la prueba de la funcion pFunc
                       1 vuelve en el array
                       0 no vuelve en el array
 *
 */
